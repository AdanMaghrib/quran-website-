 ...
 * يدعم تشغيل جميع سور القرآن الكريم بصوت الشيخ ماهر المعيقلي
...
function setupEventListeners() {
    ...
    // تحديث قائمة اختيار القارئ لتعكس استخدام ماهر المعيقلي فقط
    const reciterSelect = document.getElementById('reciter-select');
    if (reciterSelect) {
        // إزالة جميع الخيارات الحالية
        while (reciterSelect.firstChild) {
            reciterSelect.removeChild(reciterSelect.firstChild);
        }
        
        // إضافة خيار ماهر المعيقلي فقط
        const option = document.createElement('option');
        option.value = 'Maher_AlMuaiqly';
        option.textContent = 'ماهر المعيقلي';
        option.selected = true;
        reciterSelect.appendChild(option);
        
        // تعطيل القائمة المنسدلة لأنه يوجد خيار واحد فقط
...
function playSurah(surahNumber) {
    ...
    // تنسيق رقم السورة بثلاثة أرقام (001, 002, ... 114)
    const formattedSurahNumber = surahNumber.toString().padStart(3, '0');
    
    // استخدام الملفات المحلية فقط
    const audioSrc = `audio/maher/${formattedSurahNumber}.mp3`;
    ...
    const savedPosition = getSavedPosition(surahNumber);
    if (savedPosition > 0) {
 ...
 */
function getSavedPosition(surahNumber) {
